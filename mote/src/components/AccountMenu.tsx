...
export function AccountMenu() {
  ...
  return (
    <div ref={ref} className="relative">
      {open && (
        <div className="absolute bottom-full left-0 mb-2 w-[248px] overflow-hidden rounded-2xl border border-black/[.07] bg-white py-1.5 shadow-widget">
          {ITEMS.map(([label, to, sub]) => (
            <button
              ...
              className="flex w-full flex-col items-start px-3.5 py-2 text-left transition hover:bg-canvas-sunk"
            >
      ...
      <button
        ...
        className="flex w-full items-center gap-2.5 rounded-xl px-2 py-2 text-left transition hover:bg-black/[.04]"
      >
        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-shell-ink text-[11px] font-semibold text-white">
          DK
        </span>
        ...
        <span className="text-[13px] text-shell-ink/30">⋯</span>
      </button>
    </div>
  );
}
